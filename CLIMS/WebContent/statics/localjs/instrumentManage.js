...
//查看仪器信息 弹出框 --start
function moveInstrumentInfo_view(){
	//v_instrumentName
	$('#v_instrumentName').text('');
	//v_instrumentNo
	$('#v_instrumentNo').text('');
	//v_instrumentType
	$('#v_instrumentType').text('');
	//v_className
	$('#v_className').text('');
	//v_manufacturer
	$('#v_manufacturer').text('');
	...
	$('#v_price').text('');
	//v_productionDate
	$('#v_productionDate').text('');
	//v_country
	$('#v_country').text('');
	//v_instrumentManager
	$('#v_instrumentManager').text('');
	//v_managerTel
	$('#v_managerTel').text('');
	//v_dept
	$('#v_dept').text('');
...
$('.viewInstrumentCancel').click(function(){
	moveInstrumentInfo_view();
});
...
//	moveInstrumentInfo_view();  //先清空上次查看的信息
	var v_id = $(this).attr('id');
	//ajax
	$.ajax({
		url:'viewInstrument.html',
		type:'POST',
		data:{id:v_id},
		dataType:'json',
		timeout:1000,
